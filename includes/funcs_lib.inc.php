...
// midas_date_format: Updated 31 may 2006
...
function date_dropdown($pre, $selected_date = '', $start_year='', $end_year = '', $sort = 'asc',$extra='')
{
	$cur_date =	date("Y-m-d");
...
// show_thumb: Updated 16 dec 2006
function show_thumb($file_org, $width, $height, $ratio_type = 'width')
{
	if($ratio_type =='resize' ) {
		$ratio_type = 'distort';
	}
	return midas_thumb::show_thumb($file_org, $width, $height, $ratio_type);
...
{
	$result = mysql_query("show fields from $table") or die("Query failed: " . mysql_error()); 
