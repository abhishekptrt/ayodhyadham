...
class midas_pager {
	...
	var $needs_paging = true;
	 ...
	 function _pager($total_records, $records_per_page, $start='0', $theme = '') {
		$midas_pager = new midas_pager($total_records, $records_per_page, $start, $theme);
		$midas_pager->show_pager();
		return $midas_pager;
	}

	 function _displaying($total_records, $records_per_page, $start='0', $theme = '') {
		$midas_pager = new midas_pager($total_records, $records_per_page, $start, $theme);
	...
	function midas_pager($total_records, $records_per_page, $start='0', $theme = '') {
		if($records_per_page==0) {
			die("Records Per Page are 0 in pager");
		}
		if($theme == '') {
			$theme = 'default';
		}
